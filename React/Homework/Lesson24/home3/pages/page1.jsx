      ...
      return(
           <ul className="list-group">
               {this.props.users.map(function(user, index) {
                  return <ResultItem key={user.id} name={user.first_name} lastName={user.last_name} Id={user.id}>1</ResultItem>
               })}
           </ul>
      )}
})
...
var ResultItem = React.createClass({
   render: function() {
      return (
            <li className="list-group-item"><Link to={`/user/${this.props.Id}`}><button className="btn btn-lg btn-success">{this.props.name}</button></Link> {this.props.lastName}</li>
      )}
});
